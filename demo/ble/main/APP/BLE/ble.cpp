...
static void print_write_buffer(void)
{
    ...
    while (temp_spp_recv_data_node_p1 != NULL)
    {
        ESP_LOGI(GATTS_TABLE_TAG, "SPP data rx: %.*s", temp_spp_recv_data_node_p1->len,
    ...
    case ESP_GATTS_WRITE_EVT:
    {
        ...
        if (p_data->write.is_prep == false)
        {
            ...
            if (res == SPP_IDX_SPP_COMMAND_VAL)
            {
                ...
                ESP_LOGI(GATTS_TABLE_TAG, "SPP cmd rx: %.*s", p_data->write.len, (char *)p_data->write.value);
            }
            ...
            {
                ESP_LOGI(GATTS_TABLE_TAG, "SPP data rx: %.*s", p_data->write.len, (char *)p_data->write.value);
            }
            else
            {
                // TODO:
