...
#include "ble_spp_server_demo.hpp"
...
static char s_last_rx_data[128] = {0};
static bool s_rx_data_new = false;
...
void uart_task(void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        if (xQueueReceive(spp_uart_queue, (void *)&event, portMAX_DELAY))
        {
            switch (event.type)
            {
...
void spp_cmd_task(void *arg)
{
    ...
    for (;;)
    {
        ...
        {
            ESP_LOGI(GATTS_TABLE_TAG, "cmd received: %.*s", (int)strnlen((char *)cmd_id, SPP_CMD_MAX_LEN), (char *)cmd_id);
            free(cmd_id);
        }
    }
    vTaskDelete(NULL);
...
static void spp_task_init(void)
{
    ...
    xTaskCreate(spp_cmd_task, "spp_cmd_task", 3072, NULL, 10, NULL);
}
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_REG_EVT:
        ...
        ESP_LOGI(GATTS_TABLE_TAG, "BLE ready: %s", SAMPLE_DEVICE_NAME);
    ...
    case ESP_GATTS_WRITE_EVT:
    {
        ...
        if (p_data->write.is_prep == false)
        {
            ...
            if (res == SPP_IDX_SPP_COMMAND_VAL)
            {
                ...
                /* 同时存储到 LCD 显示缓冲区 */
                {
                    size_t cp_len = (p_data->write.len < sizeof(s_last_rx_data) - 1) ? p_data->write.len : sizeof(s_last_rx_data) - 1;
                    memcpy(s_last_rx_data, p_data->write.value, cp_len);
                    s_last_rx_data[cp_len] = '\0';
                    s_rx_data_new = true;
                }
            }
            else if (res == SPP_IDX_SPP_DATA_NTF_CFG)
            {
                if ((p_data->write.len == 2) && (p_data->write.value[0] == 0x01) && (p_data->write.value[1] == 0x00))
                {
                    ...
                    ESP_LOGI(GATTS_TABLE_TAG, "client connected, notify enabled");
                }
                else if ((p_data->write.len == 2) && (p_data->write.value[0] == 0x00) && (p_data->write.value[1] == 0x00))
                {
                    enable_data_ntf = false;
                    ESP_LOGI(GATTS_TABLE_TAG, "client connected, notify disabled");
                }
            }
#ifdef SUPPORT_HEARTBEAT
    ...
    case ESP_GATTS_CONNECT_EVT:
        ...
        ESP_LOGI(GATTS_TABLE_TAG, "client connected, SPP link active");
#ifdef SUPPORT_HEARTBEAT
        uint16_t cmd = 0;
    ...
    case ESP_GATTS_DISCONNECT_EVT:
        ...
        heartbeat_count_num = 0;
#endif
        ESP_LOGI(GATTS_TABLE_TAG, "advertising: %s", SAMPLE_DEVICE_NAME);
        esp_ble_gap_start_advertising(&spp_adv_params);
        break;
...
bool ble_spp_send(const uint8_t *data, size_t len)
{
    ...
    if (len > (spp_mtu_size - 3)) {
        ESP_LOGW(GATTS_TABLE_TAG, "ble_spp_send: data too long (%u > %u)", len, spp_mtu_size - 3);
        return false;
    }
    esp_err_t ret = esp_ble_gatts_send_indicate(spp_gatts_if, spp_conn_id,
        spp_handle_table[SPP_IDX_SPP_DATA_NTY_VAL], len, (uint8_t *)data, false);
    return ret == ESP_OK;
}
...
bool ble_get_received(char *buf, size_t max_len)
{
    ...
    strncpy(buf, s_last_rx_data, max_len - 1);
    buf[max_len - 1] = '\0';
    s_rx_data_new = false;
    return true;
}
...
void bleTask(void *pvParameters)
{
    ...
    if (ret) {
        ESP_LOGE(GATTS_TABLE_TAG, "%s init bluedroid failed: %s", __func__, esp_err_to_name(ret));
        goto end;
    }
    ret = esp_bluedroid_enable();
    if (ret) {
        ESP_LOGE(GATTS_TABLE_TAG, "%s enable bluedroid failed: %s", __func__, esp_err_to_name(ret));
        goto end;
    }

    /* 初始化 UART 和队列 (必须在 BLE 注册之前, 防止回调中访问空队列) */
    spp_task_init();
    ...
    ESP_LOGI(GATTS_TABLE_TAG, "BLE SPP server started, waiting for connection...");

    /* 轮询 BLE 接收数据并显示到 LCD */
    {
        char rx_buf[128];
        while (1) {
            if (ble_get_received(rx_buf, sizeof(rx_buf))) {
                ESP_LOGI(GATTS_TABLE_TAG, "BLE rx: %s", rx_buf);
                g_lcd->fill(0, 180, 320, 240, BLACK);
                g_lcd->showString(10, 190, 300, 240, 16, rx_buf, WHITE);
            }
