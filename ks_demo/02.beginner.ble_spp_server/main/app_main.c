/**
 * @file app_main.c
 * @brief BLE SPP app_main
 *
 * BLE SPP server app_main module.
 *
...
static void update_ble_display(const char *status, const char *detail)
{
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_REG_EVT:
        ...
        update_ble_display("BLE ready", SAMPLE_DEVICE_NAME);
        ...
        if (p_data->write.is_prep == false)
        {
            ...
            else if (res == SPP_IDX_SPP_DATA_NTF_CFG)
            {
                if ((p_data->write.len == 2) && (p_data->write.value[0] == 0x01) && (p_data->write.value[1] == 0x00))
                {
                    ...
                    update_ble_display("client connected", "notify enabled");
                }
                else if ((p_data->write.len == 2) && (p_data->write.value[0] == 0x00) && (p_data->write.value[1] == 0x00))
                {
                    enable_data_ntf = false;
                    update_ble_display("client connected", "notify disabled");
                }
            }
#ifdef SUPPORT_HEARTBEAT
    ...
    case ESP_GATTS_CONNECT_EVT:
        ...
        update_ble_display("client connected", "SPP link active");
#ifdef SUPPORT_HEARTBEAT
        uint16_t cmd = 0;
    ...
    case ESP_GATTS_DISCONNECT_EVT:
        ...
        heartbeat_count_num = 0;
#endif
        update_ble_display("advertising", SAMPLE_DEVICE_NAME);
        esp_ble_gap_start_advertising(&spp_adv_params);
        break;
...
void app_main(void)
{
    ...
    if (ksdiy_lvgl_lock(0))
    {
        ...
        lv_label_set_text(s_title_label, "BLE SPP server");
        ...
        lv_label_set_text(s_detail_label, SAMPLE_DEVICE_NAME);
        lv_obj_set_style_text_font(s_detail_label, &lv_font_montserrat_20, 0);
        lv_obj_align(s_detail_label, LV_ALIGN_CENTER, 0, 34);
