/**
 * @file app_lcd_s3.c
 * @brief app_lcd_s3 module
 *
 * USB Extended Display app_lcd_s3.c(app_lcd_s3.c.BaseName).
 *
...
void app_lcd_draw(uint8_t *buf, uint32_t len, uint16_t width, uint16_t height)
{
    ...
    ESP_ERROR_CHECK(esp_jpeg_decoder_one_picture((uint8_t *)buf, len, lcd_buffer[buf_index]));
    ESP_ERROR_CHECK(ksdiy_display_draw_bitmap(lcd_buffer[buf_index], EXAMPLE_LCD_H_RES, EXAMPLE_LCD_V_RES));
...
{
    ESP_LOGI(TAG, "app_lcd_init");
    ESP_ERROR_CHECK(ksdiy_display_lcd_init());

    lcd_buffer[buf_index] = (void *)calloc(1, EXAMPLE_LCD_H_RES * EXAMPLE_LCD_V_RES * 2);
    ESP_RETURN_ON_FALSE(lcd_buffer[buf_index] != NULL, ESP_ERR_NO_MEM, TAG, "alloc lcd buffer failed");
