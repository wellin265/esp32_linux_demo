 ...
 * @brief KSDIY Korvo 开发板 LVGL 显示和触摸端口驱动
 *
 * 本模块实现 LVGL 图形库在 KSDIY Korvo ESP32-S3 开发板上的移植，
 * 包括 SPI LCD 显示驱动、I2C 触摸屏驱动和 IO 扩展器（TCA9554）配置。
 *
 * 硬件连接:
 *   - LCD:  SPI 接口，GC9A01 控制器
 *   - 触摸: I2C 接口，CST816S 芯片
 *   - IO扩展: I2C 接口，TCA9554 芯片（控制 LCD 背光和电源）
 *
...
#define EXAMPLE_LVGL_TASK_STACK_SIZE (20 * 1024)
...
void ksdiy_lvgl_port_init(void)
{
    ...
    if (ret != ESP_OK)
    {
        ESP_LOGW(TAG, "Display SPI bus unavailable, continue headless: %s", esp_err_to_name(ret));
        return;
    }
    ...
    if (ret != ESP_OK)
    {
        ESP_LOGW(TAG, "Display panel IO unavailable, continue headless: %s", esp_err_to_name(ret));
        return;
    }
    ...
    if (ret != ESP_OK)
    {
        ESP_LOGW(TAG, "Display panel driver unavailable, continue headless: %s", esp_err_to_name(ret));
        return;
    }
    ...
    if (ret != ESP_OK)
    {
        ESP_LOGW(TAG, "Display reset failed, continue headless: %s", esp_err_to_name(ret));
        return;
    }
    ret = esp_lcd_panel_init(panel_handle);
    if (ret != ESP_OK)
    {
        ESP_LOGW(TAG, "Display init failed, continue headless: %s", esp_err_to_name(ret));
        return;
    }
    esp_lcd_panel_swap_xy(panel_handle, true);
    esp_lcd_panel_mirror(panel_handle, false, true);
