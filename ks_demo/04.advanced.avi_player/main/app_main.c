 ...
 * @brief AVI 视频播放器主程序
 *
 * 从 SPIFFS 文件系统读取 AVI 视频文件并通过 LCD 播放：
 * - MJPEG 视频解码 → RGB565 → LCD 显示
 * - PCM S16LE 音频 → ES8311 编解码器播放
 * - 自动循环播放多个视频文件
 *
 * 视频格式要求：
 *   - 视频编码：MJPEG
...
static const char *TAG = "avi_player";

/* ========== 播放控制变量 ========== */

/** @brief 当前播放的视频文件序号 */
static uint8_t s_video_index = 93;
...
/**
 * @brief 播放结束回调 —— 自动播放下一个视频
 */
static void on_avi_play_end(void *arg)
{
    ESP_LOGI(TAG, "播放结束，切换下一个");

    s_video_index++;
    if (s_video_index > 107) {
        s_video_index = 94;  /* 循环 h0094 ~ h0107 */
    }

    snprintf(s_file_name, sizeof(s_file_name), "/spiffs/h0%03d.avi", s_video_index);
    avi_player_play_from_file(s_file_name);
}
