...
static bool load_lottie_from_path(lv_obj_t *canvas, const char *path)
{
    ...
    lv_lottie_set_src_data(canvas, loaded_lottie_data, loaded_lottie_size);
...
static void initial_lottie_task(void *arg)
{
    ...
    if (initial_lottie_path[0] != '\0' && ui_face_canvas != NULL && ksdiy_lvgl_lock(-1)) {
...
static void image_display(void)
{
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (initial_lottie_path[0] == '\0') {
            snprintf(initial_lottie_path, sizeof(initial_lottie_path), "/spiffs/%s", entry->d_name);
        }
    }
